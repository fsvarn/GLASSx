###################################################
# Create scatterplot correlating the mesenchymal myeloid score with bulk mesenchymal tumor score in single-cell data
...
tcga_sig <- rownames(tcga_dat %>% filter(q.val < 0.05 & eff > log2(1.5)))
...
cor(plot_res$mean, plot_res$enrichment_score/1000)
...
strip.background = element_blank(),
legend.position="none") +
scale_x_continuous(breaks = seq(0, 0.24, by = 0.02), labels = scales::number_format(accuracy = 0.01))
dev.off()
