...
for(i in 1:length(myinf1))
{
  ...
  denom <- sum(imm[,"Sox2"] >= thr["Sox2"] & ((imm[,"CD44"] >= thr["CD44"] | imm[,"EGFR"] >= thr["EGFR"]) | imm[,"Olig2"] >= thr["Olig2"] | imm[,"Ki67"] >= thr["Ki67"]))
  ...
  counts[i] <- sum(diff_count, stem_count, prolifstem_count)
  total[i] <- denom
}

sum(counts == total) #8, all cells accounted for

if_pct <- data.frame(mytag, tumor_diff, tumor_stem, tumor_prolifstem)
if_pct <- if_pct %>%
...
tumor_dat <- dat %>%
  filter(cell_state %in% c("differentiated_tumor", "stemcell_tumor", "prolif_stemcell_tumor")) %>%
  ...
  filter(!(aliquot_barcode %in% c("GLSS-LU-0B12-R1-01R-RNA-3P44CC")))

cor(tumor_dat[,2:4], tumor_dat[,5:ncol(tumor_dat)],method="p")
cor(tumor_dat[,2:4], tumor_dat[,5:ncol(tumor_dat)],method="s")
...
aliquot_barcode <- rep(tumor_dat$aliquot_barcode, 3)
if_dat <- c(tumor_dat$tumor_stem, tumor_dat$tumor_prolifstem, tumor_dat$tumor_diff)
csx_dat <- c(tumor_dat$stemcell_tumor, tumor_dat$prolif_stemcell_tumor, tumor_dat$differentiated_tumor)
cell_state <- rep(c("Stem-like", "Prolif. stem-like","Diff.-like"), each = nrow(tumor_dat))

plot_dat <- data.frame(aliquot_barcode, if_dat, csx_dat, cell_state)

pdf("/Users/varnf/Desktop/Figs/GLASS-III/revisions/tumor_cor.pdf", height = 3, width = 3)
...
pdf("/Users/varnf/Desktop/Figs/GLASS-III/revisions/tumor_cor_split.pdf",width=3,height=1.3)
ggplot(plot_dat, aes(x=if_dat*100,y=csx_dat*100)) +
