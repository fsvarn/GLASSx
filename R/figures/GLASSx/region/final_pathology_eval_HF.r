...
# Remove IDHmut
#dat <- dat[-which(dat$sample_barcode %in% c("GLSS-HF-4B46-R1", "GLSS-HF-4B46-TP", "GLSS-HF-4F0A-R2", "GLSS-HF-4F0A-TP", "GLSS-HF-CB66-R2", "GLSS-HF-CB66-TP")),]
...
pdf("figures/pathology/HF_pathology_concordance.pdf",height = 4, width = 6.5)
...
pdf("figures/pathology/HF_pathology_cov.pdf",height = 4, width = 4)
...
pdf("figures/pathology/HF_pathology_lecomb_cov.pdf",height = 4, width = 4)
ggplot(path_coll, aes(x = feature, y = cov)) +
  geom_hline(yintercept = 1, colour = "gray") +
  geom_boxplot() +
  theme_bw() +
...
write.table(mean_pathologist, "data/pathology_annot/mean_HF_results_for_laila.txt", sep="\t", quote=FALSE, row.names=FALSE)
...
pdf("figures/pathology/HF_mean_correlations.pdf",height = 2, width = 3.75)
...
pdf("figures/pathology/total_necrosis_cor_glass_legend_HF.pdf", height = 2, width = 2)
...
pdf("figures/pathology/total_necrosis_cor_glass_HF.pdf", height = 2, width = 2)
ggplot(data = plot_nec, aes(x = total_necrosis, y = CTpan*100)) +
  geom_point(aes(shape = timepoint)) +
  geom_smooth(method="lm",se = FALSE) +
  labs(x = "Necrosis % (Pathology)", y = "PAN % (CIBERSORT)") +
...
pdf("figures/pathology/necrosis_cor_glass_HF.pdf", height = 2, width = 2)
...
ggplot(plot_nonec %>% filter(folder=="HF",timepoint=="Primary"), aes(x=pathology,y=cibersort, colour=feature)) +
  geom_point(size=1) +
  geom_smooth(method='lm',se=FALSE) +
...
full_comp_nonec %>%
  filter(folder == "HF", timepoint == "Primary") %>%
  group_by(pathologist) %>%
  select_if(is.numeric) %>%
  group_map(~correlate(.x)) 
