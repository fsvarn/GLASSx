...
together AS
(
SELECT aliquot_barcode, COALESCE(AVG(hla_loss),500) AS loss_mean, COUNT(hla_loss) AS loss_count, COALESCE(AVG(hla_kept),500) AS kept_mean, COUNT(hla_kept) AS kept_count,
...
dat %>%
group_by(idh_status) %>%
summarise(pval = wilcox.test(as.numeric(loss_mean),as.numeric(kept_mean),paired=TRUE)$p.value,
		  eff = median(loss_mean,na.rm=TRUE) - median(kept_mean,na.rm=TRUE))	#P = 0.57
...
avg_affinity <- as.numeric(c(dat[,"kept_mean"],dat[,"loss_mean"]))
aliquot_barcode <- rep(dat[,"aliquot_barcode"],2)
timepoint <- rep(dat[,"timepoint"],2)
idh_status <- rep(dat[,"idh_status"],2)
plot_dat <- data.frame(aliquot_barcode,status,avg_affinity,timepoint,idh_status)

plot_dat <- plot_dat %>%
mutate(idh_status = fct_relevel(idh_status, "IDHwt", "IDHmut"))

pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/hla_neoag_affin_loss_v_kept.pdf",width=2,height=1.8)
ggplot(data = plot_dat, aes(x = status, y = avg_affinity, colour= timepoint)) +
...
labs(y = "Average binding affinity (NetMHCpan)") +
