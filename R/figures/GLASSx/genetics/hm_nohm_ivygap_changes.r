...
JOIN analysis.cibersortx_ivygap ci1 ON ci1.aliquot_barcode = ps.rna_barcode_a
JOIN analysis.cibersortx_ivygap ci2 ON ci2.aliquot_barcode = ps.rna_barcode_b AND ci2.cell_state = ci1.cell_state
...
hm_only <- dat %>% filter(hm == 1, cell_state == "CTmvp")
