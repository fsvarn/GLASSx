...
cor(frac[,"differentiated_tumor"], frac[,"mes_like_sox2_cd44"] + frac[,"ac_like_sox2_egfr"])
cor(frac[,"stemcell_tumor"],frac[,"stem_like_sox2_olig2"])
cor(frac[,"prolif_stemcell_tumor"],frac[,"prolif_stem_like_sox2_olig2_ki67"])
cor(frac[,"myeloid"], frac[,"CD14"])
cor(frac[,"differentiated_tumor"] + frac[,"stemcell_tumor"] + frac[,"prolif_stemcell_tumor"], frac[,"Sox2"])

cor(frac[,"differentiated_tumor"], frac[,"Sox2"] - frac[,"stem_like_sox2_olig2"] - frac[,"prolif_stem_like_sox2_olig2_ki67"])
...
imm <- c(frac$mes_like_sox2_cd44 + frac$ac_like_sox2_egfr, frac$stem_like_sox2_olig2, frac$prolif_stem_like_sox2_olig2_ki67, frac$CD14, frac$Sox2)
...
ggplot(data = plot_dat, aes(x = imm, y = cibersortx*100)) +
...
ggplot(data = sub_dat, aes(x = imm, y = cibersortx*100)) +
