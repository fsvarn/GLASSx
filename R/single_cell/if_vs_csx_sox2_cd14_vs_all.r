...
for(i in 1:length(myinf1))
{
  ...
  denom <- sum(imm[,"Sox2"] >= thr["Sox2"] | imm[,"CD14"] >= thr["CD14"])
  tumor_counts <- sum(imm[,"Sox2"] >= thr["Sox2"] & imm[,"CD14"] < thr["CD14"])
  myeloid_counts <- sum( imm[,"CD14"] >= thr["CD14"])
  SOX2[i] <- tumor_counts/denom
  CD14[i] <-  myeloid_counts/denom
  
  counts[i] <- sum(tumor_counts, myeloid_counts)
  total[i] <- denom
}
sum(counts == total)

if_pct <- data.frame(mytag, SOX2, CD14)
apply(if_pct[,2:3], 1, sum)
...
tumor_dat <- dat %>%
  ...
  mutate(tumor = sum(differentiated_tumor, stemcell_tumor, prolif_stemcell_tumor), myeloid = sum(myeloid, granulocyte, dendritic_cell)) %>%
  select(aliquot_barcode, tumor, myeloid) %>%
...
cor(tumor_dat[,2:3], tumor_dat[,4:ncol(tumor_dat)],method="p")
cor(tumor_dat[,2:3], tumor_dat[,4:ncol(tumor_dat)],method="s")
