...
myinf1 <- "data/pathology_annot/Histo_review_aggregate.txt"
...
# Plot pathologist agreement
samples <- as.character(unique(dat_long$CASES))
for(i in 1:length(samples))
{
  out_file <- paste("figures/pathology/",samples[i],".pdf",sep="")
  if(grepl("-TP",samples[i])){
      p1 = ggplot(data = dat_long %>% 
                    filter(CASES == samples[i], feat_class == "Primary"),
             aes(x = feature, y = metric)) +
        geom_boxplot(colour="black") +
        geom_point(size=1) +
        ggtitle(samples[i]) +
        facet_wrap(.~metric_class,scales = "free") +
...
dat_avg <- dat_long %>%
           group_by(CASES, feature) %>%
           summarise(mean = mean(metric), sd = sd(metric))
...
pri_percent <- dat_long %>% 
               filter(metric_class == "Percent", feat_class == "Primary", grepl("-TP", CASES)) %>%
               group_by(CASES,Pathologist) %>%
...
rec_percent <- dat_long %>% 
  filter(metric_class == "Percent", feat_class == "Primary", grepl("-R1", CASES)) %>%
  group_by(CASES,Pathologist) %>%
  summarise(metric = sum(metric))
...
ivy <- ivy %>%
       filter(grepl("GLSS-LU", aliquot_barcode)) 
...
pdf("figures/pathology/heatmap_cor.pdf", height = 6, width = 6)
...
pdf("figures/pathology/feature_cor_with_nec.pdf", height = 3, width = 4)
...
cor_plot <- cor_plot %>% mutate(status = fct_relevel(status, "TP","R1"))
pri_plot <- cor_plot %>% filter(status == "TP")
rec_plot <- cor_plot %>% filter(status == "R1")
...
comp_plot <- comp_plot %>%
             mutate(status = fct_relevel(status, "TP", "R1")) %>%
             mutate(feature = fct_relevel(feature, rev(unique(feature))))
...
g1 <- full_comp[which(full_comp$Necrosis_score > thr3),"mes"]
g2 <- full_comp[which(full_comp$Necrosis_score <= thr3),"mes"]
wilcox.test(g1,g2)
