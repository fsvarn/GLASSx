...
hits <- res %>% filter(ligand_cor > 0, receptor_cor > 0, ligand_q < 0.1, receptor_q < 0.1)
...
pass_join <- pass %>%
		...
		select(ligand, receptor, ligand_source, receptor_source, 
...
pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/scgp_mes_rec_lig_screen_v2.pdf",width=1.6,height=2.95)
ggplot(plot_res2, aes(ligand_cor, receptor_cor, colour = highlight)) + 
geom_point()  +
geom_hline(yintercept=0, colour = "gray") +
geom_vline(xintercept=0, colour = "gray") +
scale_colour_manual(values = c("TRUE" = "#CD4F39", "FALSE" = "black")) +
...
# Test MARCO
myeloid_cpm <- t(log2cpm["MARCO",rownames(myeloid_cells)]) %>% data.frame()
myeloid_cpm[,"sample_id"] <- myeloid_cells[,"sample_id"]
colnames(myeloid_cpm) <- c("cpm","sample_id")
...
tumor_cpm <- t(log2cpm["SCGB3A2",rownames(tumor_cells)]) %>% data.frame()
tumor_cpm[,"sample_id"] <- tumor_cells[,"sample_id"]
colnames(tumor_cpm) <- c("cpm","sample_id")
...
# Plot result
marcores <- myeloid_cpm %>%
group_by(sample_id) %>%
summarise(mean_cpm = mean(cpm)) %>%
inner_join(bulk_mes, c("sample_id" = "sample_id"))

pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/marco_scatterplot.pdf",width=1.9,height=1.5)  
ggplot(marcores, aes(x = mean_cpm, y = enrichment_score/1000)) +
geom_point() +
geom_smooth(method = lm, se = FALSE) + 
theme_classic() +
labs(x = "Mean MARCO log2(CPM)", y = "Bulk mes. score") +
