...
myinf1 <- "/projects/verhaak-lab/GLASS-III/results/cibersortx/hires/GLASS-freeze_validation/CIBERSORTxGEP_GLASS_SM_GEPs_HeatMap.txt"

mat <- read.delim(myinf1)

colnames(mat)[1] <- "gene"
colnames(mat)[2:7] <- paste("cibersortx_", colnames(mat)[2:7], sep = "")
colnames(mat)[8:13] <- paste("klemm_", colnames(mat)[8:13], sep = "")
...
sub_mat2 <- sub_mat2[-which(non_tumor_myeloid),]
