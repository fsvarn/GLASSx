###################################################
# Compare CIBERSORTx profiles in subtype switches after histological feature adjustment
...
summarise(pval = t.test(fraction_a, fraction_b, paired=TRUE)$p.value, eff = mean(fraction_b - fraction_a), n = n()) %>% 
...
geom_tile() +
scale_fill_gradient2(low ="royalblue4", mid = "white", high = "tomato3", midpoint = 0, limits = c(-0.255,0.255)) +
...
pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/malig_subtype_switch_unadj_legend_v2.pdf",width=2.735,height=1.5)
ggplot(unadj_res %>% filter(idh_status == "IDHwt"), aes(x = subtype_a, y = subtype_b, fill=eff)) +
geom_tile() +
scale_fill_gradient2(low ="royalblue4", mid = "white", high = "tomato3", midpoint = 0, limits = c(-0.255,0.255)) +
facet_grid(. ~ cell_state) +
...
ungroup() %>%
group_by(cell_state, idh_status, subtype_a, subtype_b) %>%
summarise(pval = t.test(new_fraction_a, new_fraction_b, paired=TRUE)$p.value, eff = mean(new_fraction_b - new_fraction_a,na.rm=TRUE), n = n()) %>% 
mutate(logp = log10(pval), status = case_when(
pval < 0.05 & eff > 0 ~ "up",
pval < 0.05 & eff < 0 ~ "dn",
pval > 0.05 ~ "none")) %>%
...
pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/malig_subtype_switch_adj_LE_legend_v2.pdf",width=2.735,height=1.5)
ggplot(adj_res %>% filter(idh_status == "IDHwt"), aes(x = subtype_a, y = subtype_b, fill=eff)) +
geom_tile() +
scale_fill_gradient2(low ="royalblue4", mid = "white", high = "tomato3", midpoint = 0, limits = c(-0.255,0.255)) +
facet_grid(. ~ cell_state) +
...
pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/pmt_stacked_ladder_barplot_adj_LE.pdf",width=1.9,height=1.5)
...
pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/malig_subtype_switch_adj_nonCT_v2.pdf",width=2.735,height=1.5)
ggplot(adj_res %>% filter(idh_status == "IDHwt"), aes(x = subtype_a, y = subtype_b, fill=eff)) +
geom_tile() +
scale_fill_gradient2(low ="royalblue4", mid = "white", high = "tomato3", midpoint = 0, limits = c(-0.255,0.255)) +
facet_grid(. ~ cell_state) +
...
pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/malig_subtype_switch_adj_nonCT_legend_v2.pdf",width=2.735,height=1.5)
ggplot(adj_res %>% filter(idh_status == "IDHwt"), aes(x = subtype_a, y = subtype_b, fill=eff)) +
geom_tile() +
scale_fill_gradient2(low ="royalblue4", mid = "white", high = "tomato3", midpoint = 0, limits = c(-0.255,0.255)) +
facet_grid(. ~ cell_state) +
