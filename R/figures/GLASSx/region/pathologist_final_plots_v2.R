...
# Correlate everything against everything, split by pathologist
cor_dat <- long_dat %>%
...
pdf("figures/pathology/mean_pathologist_cor_rev_LU.pdf",height = 1.7, width = 3.25)
...
pdf("figures/pathology/mean_pathologist_cor_LU",height = 2.2, width = 3)
...
pdf("figures/pathology/mean_pathologist_rec_correlation_LU.pdf",height = 2.2, width = 2.3)
...
median_nec_cor <- cor_nec %>%
  group_by(folder, timepoint, path_feature, cibersort_feature) %>%
  summarise(cor = median(cor)) 

median_nec_cor_all <- cor_nec %>%
  group_by(folder, path_feature, cibersort_feature) %>%
  summarise(cor = median(cor)) 
...
pdf("figures/pathology/mean_pathologist_necrosis_cor_legend.pdf",height = 2, width = 2)
...
pdf("figures/pathology/mean_pathologist_necrosis_cor.pdf",height = 2, width = 2)
ggplot(data = plot_nec, aes(x = path_percent, y = cibersort_percent*100)) +
  geom_point(aes(shape = timepoint)) +
  geom_smooth(method="lm",se = FALSE) +
  labs(x = "Necrosis % (Pathology)", y = "CTpan % (CIBERSORT)") +
