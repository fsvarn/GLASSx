...
# 	elimRes[,"logp"] <- -log10(as.numeric(elimRes$raw.p.value))
	...
	# Get genes in term
	allGO[[i]] = genesInTerm(sampleGOdata)
}
...
plotRes <- goRes %>% filter(cell_state=="stemcell_tumor")
plotRes <- plotRes[1:10,]

		   
# Stem-like cell heatmap
term <- unique(plotRes$GO.ID)
term2 <- unique(plotRes$Term)
...
#mygenes <- unique(unlist(allGO[[3]][term]))

#sig_matrix <- geps[mygenes,c(dat[,"tumor_barcode_a"], dat[,"tumor_barcode_b"])]
#sig_matrix <- t(apply(sig_matrix, 1, function(x) (x - mean(x))/sd(x)))
# 
# score <- apply(sig_matrix,2,function(x)mean(x,na.rm=TRUE))
# score <- score[order(score,decreasing=TRUE)]

init_score <- rec_score <- rep(0,length())
for(i in 1:length(term))
{
	cat("\r", i)

	mygenes <- allGO[[3]][term[i]][1]]
	
	# Plot heatmaps
	sig_matrix <- geps[mygenes,c(dat[,"tumor_barcode_a"], dat[,"tumor_barcode_b"])]
	
	sig_matrix <- t(apply(sig_matrix, 1, function(x) (x - mean(x))))
	
	grp1 <- sig_matrix[,dat[,"tumor_barcode_a"]]
	grp1 <- mean(apply(grp1, 2, mean))
	grp2 <- sig_matrix[,dat[,"tumor_barcode_b"]]
	grp2 <- mean(apply(grp2, 2, mean))
	
	init_score[i] <- grp1
	rec_score[i] <- grp2
}

score <- c(init_score, rec_score)
goterm <- rep(term2,2)
timepoint <- rep(c("Initial","Recurrent"),each=length(term))

plot_res <- data.frame(goterm, score, timepoint)

pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/stemlike_goterm_heatmap.pdf",width=2.7,height=1.5)
ggplot(data = plot_res, aes(x = timepoint, y = goterm)) +
geom_tile(aes(fill=score)) +
scale_fill_gradient2(low ="royalblue4", mid = "white", high = "tomato3",
space = "Lab", na.value = "grey50", guide = "colourbar",
aesthetics = "fill", limits = c(-0.05, 0.05)) + 
theme_void() +
