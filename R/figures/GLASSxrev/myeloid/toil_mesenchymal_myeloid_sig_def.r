###################################################
# Define the mesenchymal myeloid signature and perform GO enrichment
...
ggplot(data = heatmap_res %>% filter(aliquot_barcode=="TCGA-02-0047-01"), aes(x = logp, y = gene)) +
geom_bar(stat="identity") +
...
plotRes <- fishRes %>% 
		   ...
		   filter(q.value < 0.05 & Significant > Expected & Expected > 1) # Prioritize gene sets whose ratios are not due to one expected gene

plotRes[,"logp"] <- -log10(as.numeric(plotRes$q.value))


plotRes2 <- plotRes[1:10,]

plotRes2 <- plotRes2 %>%
mutate(Term = recode(Term, "chemokine-mediated signaling pathway" = "chemokine-med. sign. pathway"))
		   
# Plot the results
pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/tcga_mes_sig_functional_enrichment.pdf", width = 2.4, height = 1.55) #,width=4,height=2.5)
ggplot(data = plotRes2, aes(x = logp, y = Term)) +
geom_bar(stat="identity") +
