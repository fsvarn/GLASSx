###################################################
# Compile differential expression results together to make supplemental tables
...
write.table(long_res, "/projects/verhaak-lab/GLASS-III/data/res/CIBERSORTx/analysis/SuppTable_posttreatment_sigs_rev.txt", sep = "\t", quote=FALSE, row.names=FALSE)

#---------------------------------------------


#  Read in post-treatment stem cell signature
myinf1 <- "/projects/verhaak-lab/GLASS-III/data/res/CIBERSORTx/analysis/TCGA_myeloid_ts_result_v4.txt"
mysig <- read.delim(myinf1)

mysig <- mysig %>% filter(q.val < 0.05, eff > log2(1.5)) %>% rownames_to_column(var="gene_symbol")
mysig <- mysig[,1:4]
mysig$eff <- 2^mysig$eff

write.table(mysig, "/projects/verhaak-lab/GLASS-III/data/res/CIBERSORTx/analysis/SuppTable_mes_myeloid_sig_rev.txt", sep = "\t", quote=FALSE, row.names=FALSE)
