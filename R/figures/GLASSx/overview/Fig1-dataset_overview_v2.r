...
plot_res[,"codel"] <- as.character(as.numeric(plot_res[,"idh_codel_subtype"] == "IDHmut-codel"))
...
gg_simplicity_score <-
  ggplot(plot_res %>% filter(signature_name=="Mesenchymal", cell_state == "Stem-like"), aes(x=case_barcode)) +
  geom_tile(aes(y=status, alpha= -1 * ss)) +
  scale_fill_manual(values=c("black")) +
  theme(axis.title.y=element_blank())
  
gg_transcript_subtype_pro <-
  ggplot(plot_res %>% filter(signature_name=="Proneural", cell_state == "Stem-like"), aes(x=case_barcode)) +
  geom_tile(aes(y=status, fill=signature_name, alpha= pval)) +
  scale_fill_manual(values=c("#00458a")) +
  theme(axis.title.y=element_blank())
  
gg_transcript_subtype_cla <-
  ggplot(plot_res %>% filter(signature_name=="Classical", cell_state == "Stem-like"), aes(x=case_barcode)) +
  geom_tile(aes(y=status, fill=signature_name, alpha= pval)) +
...
gg_codel <- 
  ...
  geom_tile(aes(y=signature_name, fill=codel)) +
...
gg_grade_change <- 
  ggplot(plot_res %>% filter(status=="Recurrent",signature_name=="Proneural", cell_state == "Stem-like"), aes(x=case_barcode)) +
...
n12 <- length(gb12$layout$panel_scales_y[[1]]$range$range)
...
g <- gtable:::rbind_gtable(g, gL, "last")

panels <- g$layout$t[grep("panel", g$layout$name)]
g$heights[panels[1*2]] <- unit(n1, "null")
g$heights[panels[2*2]] <- unit(n1, "null")
g$heights[panels[3*2]] <- unit(n1,"null")
g$heights[panels[4*2]] <- unit(n1,"null")
g$heights[panels[5*2]] <- unit(n1*3.2,"null")
g$heights[panels[6*2]] <- unit(n1*3.2,"null")
g$heights[panels[7*2]] <- unit(n1/2,"null")
g$heights[panels[8*2]] <- unit(n1/2,"null")
g$heights[panels[9*2]] <- unit(n1/2,"null")
g$heights[panels[10*2]] <- unit(n1/2,"null")
g$heights[panels[11*2]] <- unit(n1/2,"null")
g$heights[panels[12*2]] <- unit(n1/2,"null")
...
barplot_res <- plot_res %>% 
			   group_by(cell_state, status, idh_status) %>% 
			   summarise(proportion = mean(proportion)) %>%
			   mutate(status = as_factor(status)) %>%
		       mutate(status = fct_relevel(status, "Initial","Recurrent"))
			   

mysubtype <- unique(plot_res[,"idh_status"])   
...
for(i in 1:length(mycellstate))
{
	...
	{
		dat1 <- plot_res %>% filter(cell_state == mycellstate[i] & idh_status == mysubtype[j] & status == "Initial")
		dat2 <- plot_res %>% filter(cell_state == mycellstate[i] & idh_status == mysubtype[j] & status == "Recurrent")
