...
iv.cell_state AS ivygap_region, iv.fraction AS ivygap_fraction,
CASE WHEN cs.idh_codel_subtype = 'IDHwt' THEN 'IDHwt' ELSE 'IDHmut' END AS idh_status
FROM analysis.cibersortx_scgp sc
JOIN analysis.cibersortx_ivygap iv ON iv.aliquot_barcode = sc.aliquot_barcode
...
dat <- dat %>% 
		...
		mutate(ivygap_region = recode(ivygap_region, 
...
res <- dat %>% 
		group_by(scgp_cell_state, ivygap_region, idh_status) %>% 
		summarise(cor = cor(scgp_fraction, ivygap_fraction, method="p"), pval = cor.test(scgp_fraction, ivygap_fraction, method="p")$p.value) %>%
		mutate(idh_status = fct_relevel(idh_status, "IDHwt","IDHmut")) %>%
		as.data.frame()
		
pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/scgp_ivygap_cor.pdf", width=6, height = 1.7) #,width=2.7,height=3)
ggplot(res, aes(x = scgp_cell_state, y = ivygap_region, fill=cor)) +
geom_tile() +
