...
path_dat <- dat %>% 
            ...
            mutate(sample_barcode = fct_relevel(sample_barcode, c("GLSS-LU-00B9-TP", "GLSS-LU-00B9-R1",
                                                  "GLSS-LU-00C2-TP", "GLSS-LU-00C2-R1",
                                                  "GLSS-LU-0B10-TP", "GLSS-LU-0B10-R1",
                                                  "GLSS-LU-0B12-TP", "GLSS-LU-0B12-R1",
                                                  "GLSS-LU-0B13-TP", "GLSS-LU-0B13-R1")))
...
pdf("figures/pathology/LU_pathology_concordance.pdf",height = 4, width = 6.5)
ggplot(path_dat %>% filter(sample_barcode %in% c("GLSS-LU-00B9-TP", "GLSS-LU-00B9-R1", "GLSS-LU-00C2-TP", "GLSS-LU-00C2-R1")), aes(x = feature, y = fraction)) +
  geom_boxplot() +
...
pdf("figures/pathology/LU_pathology_cov.pdf",height = 4, width = 4)
...
pdf("figures/pathology/LU_pathology_lecomb_cov.pdf",height = 4, width = 4)
ggplot(path_coll, aes(x = feature, y = cov)) +
  geom_hline(yintercept = 1, colour = "gray") +
  geom_boxplot() +
  theme_bw() +
...
pdf("figures/pathology/LU_mean_correlations.pdf",height = 2, width = 3.75)
...
ggplot(plot_nonec %>% filter(folder=="LU",timepoint=="Primary"), aes(x=pathology,y=cibersort, colour=feature)) +
  geom_point(size=1) +
  geom_smooth(method='lm',se=FALSE) +
...
full_comp_nonec %>%
  filter(folder == "LU", timepoint == "Primary") %>%
  group_by(pathologist) %>%
  select_if(is.numeric) %>%
  group_map(~correlate(.x)) 
