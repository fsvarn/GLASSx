###################################################
# Create PCA/UpSet plots examining differences in myeloid expression across subtypes
# Author: Frederick Varn
# Date: 2022.01.19
# Figures S5E, S5H, Table S7
##################################################
...
# PCA density plot
pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/CIBERSORTx_toil_myeloid_pca_dens.pdf",width=2.1,height=2.5)
...
geps <- read.delim(myinf1, row.names=1)
colnames(geps) <- gsub("\\.","-",colnames(geps))
#geps <- log10(geps+1)		No need to log-transform because of fold-change
#gep_list <- t(geps)
...
for(i in 1:length(mysubtype))
{
	...
	idhwt_res$subtype <- mysubtype[i]
	idhwt_res$idh_status <- "IDH-wild-type"
...
# Long table
long_res <- do.call(rbind, diff_list)
long_res$gene_symbol <- rownames(long_res)
long_res$gene_symbol <- sapply(strsplit(long_res$gene_symbol, "\\."),function(x)paste(x[2:length(x)],collapse="."))
long_res <- long_res %>%
 			filter(eff > log2(1.5), sig) %>%
 			dplyr::select(gene_symbol, p.val, q.val, eff, subtype, idh_status)

write.table(long_res, "/projects/verhaak-lab/GLASS-III/data/res/myeloid_overlap/idhwt_subtype_myeloid_table_long.txt", sep = "\t", quote=FALSE, row.names=FALSE)

# Identify gene sets
cla_genes <- rownames(diff_list[["Classical"]] %>% filter(eff > log2(1.5), sig))
mes_genes <- rownames(diff_list[["Mesenchymal"]] %>% filter(eff > log2(1.5), sig))
pro_genes <- rownames(diff_list[["Proneural"]] %>% filter(eff > log2(1.5), sig))

upset_list <- list(cla_genes, mes_genes, pro_genes)
names(upset_list) <- mysubtype
pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/idhwt_upset.pdf", width = 2.6, height = 2)  
...
# Identify gene sets
idhwt_genes <- rownames(diff_list[["IDHwt"]] %>% filter(eff > log2(1.5), sig))
noncodel_genes <- rownames(diff_list[["IDHmut-non-codel"]] %>% filter(eff > log2(1.5), sig))
codel_genes <- rownames(diff_list[["IDHmut-codel"]] %>% filter(eff > log2(1.5), sig))
...
mdm_sig <- sigs %>%
		filter(signature_set == "Muller", signature_name == "Macrophages")

intersect(mdm_sig[,"gene_symbol"], basal_sig)
intersect(mdm_sig[,"gene_symbol"], clames)

mg_sig <- sigs %>%
		filter(signature_set == "Muller", signature_name == "Microglia")
intersect(mg_sig[,"gene_symbol"], basal_sig)
intersect(mg_sig[,"gene_symbol"], clames)		
