...
(
	...
	hla_type1, hla_type2, hla_type1_copy_number, hla_type2_copy_number, pval, loss_allele, kept_allele,coverage_filter
	FROM variants.titan_seg ts
	JOIN variants.lohhla lh ON lh.pair_barcode = ts.pair_barcode
	WHERE chrom = 6 AND pos @> int4range(29909037, 29913661) AND hla_type1 LIKE 'HLA-A%' ORDER BY 1
),
hla_b AS
(
	SELECT ts.pair_barcode, ts.chrom, ts.pos, ts.median_ratio, ts.median_logr, ts.corrected_call, ts.copy_number, ts.major_cn, ts.minor_cn,
	...
	FROM variants.titan_seg ts
	JOIN variants.lohhla lh ON lh.pair_barcode = ts.pair_barcode
	WHERE chrom = 6 AND pos @> int4range(31321649,31324964) AND hla_type1 LIKE 'HLA-B%' ORDER BY 1
),
hla_c AS
(
	SELECT ts.pair_barcode, ts.chrom, ts.pos, ts.median_ratio, ts.median_logr, ts.corrected_call, ts.copy_number, ts.major_cn, ts.minor_cn,
	...
	FROM variants.titan_seg ts
	JOIN variants.lohhla lh ON lh.pair_barcode = ts.pair_barcode
	WHERE chrom = 6 AND pos @> int4range(31236526,31239869) AND hla_type1 LIKE 'HLA-C%' ORDER BY 1
),
together AS
(
	SELECT * FROM hla_a
...
SELECT tg.*
FROM together tg
JOIN analysis.pairs pa ON pa.pair_barcode = tg.pair_barcode
JOIN analysis.lohhla_set gs ON pa.tumor_barcode = gs.tumor_barcode_a OR pa.tumor_barcode = gs.tumor_barcode_b
WHERE coverage_filter = covfilt AND pval IS NOT NULL
ORDER BY 10,1
"


cov_filt <- c(5,10,20,30)
res <- matrix(0,nrow=length(cov_filt),ncol=6)
rownames(res) <- cov_filt
colnames(res) <- c("major_cn_cor","minor_cn_cor","all_cor","lohhla_in_titan","titan_in_lohhla","num")
for(i in 1:length(cov_filt))
{
	...
	res[i,"major_cn_cor"] <- cor(major_cn,dat[,"major_cn"],method="s")
	res[i,"minor_cn_cor"] <- cor(minor_cn,dat[,"minor_cn"],method="s")
	res[i,"all_cor"] <- cor(c(major_cn,minor_cn),c(dat[,"major_cn"],dat[,"minor_cn"]),method="s")

	titan_loss <- dat[grep("LOH",dat[,"corrected_call"]),]
	res[i,"lohhla_in_titan"] <- sum(titan_loss[,"pval"]<0.1 & (titan_loss[,"hla_type1_copy_number"] < 0.5 | titan_loss[,"hla_type2_copy_number"] < 0.5))/nrow(titan_loss)

	hla_loss <- dat[which(dat[,"pval"]<0.1 & (dat[,"hla_type1_copy_number"] < 0.5 | dat[,"hla_type2_copy_number"] < 0.5)),]
	res[i,"titan_in_lohhla"] <- nrow(hla_loss[grep("LOH",hla_loss[,"corrected_call"]),])/nrow(hla_loss)
	
	res[i,"num"] <- nrow(dat)
	
# 	titan_loss <- dat[grep("LOH",dat[,"corrected_call"]),]
# 	res[i,"lohhla_in_titan_loh"] <- sum(titan_loss[,"hla_type1_copy_number"] < 0.5 | titan_loss[,"hla_type2_copy_number"] < 0.5)/nrow(titan_loss)
# 
# 	titan_ab <- dat[which(dat[,"major_cn"] > dat[,"minor_cn"]),]
# 	res[i,"lohhla_in_titan_ab"] <- sum(titan_ab[,"pval"] < 0.1)/nrow(titan_ab)
# 
# 	hla_loss <- dat[which(dat[,"hla_type1_copy_number"] < 0.5 | dat[,"hla_type2_copy_number"] < 0.5),]
# 	res[i,"titan_in_lohhla_loh"] <- nrow(hla_loss[grep("LOH",hla_loss[,"corrected_call"]),])/nrow(hla_loss)
# 
# 	hla_ab <- dat[which(dat[,"major_cn"] > dat[,"minor_cn"]),]
# 	res[i,"titan_in_lohhla_ab"] <- sum(hla_ab[,"pval"] < 0.1)/nrow(hla_ab)
}
