###################################################
# Build a differentiated tumor cell expression heatmap from Xue macrophage profiles split up by IDHmut subtype
# Updated: 2020.07.17
...
#######################################################
rm(list=ls())
myinf1 <- "/projects/verhaak-lab/GLASS-III/results/cibersortx/hires/CIBERSORTxHiRes_GLASS_differentiated_tumor_Window48.txt"
...
pdf("/projects/varnf/GLASS-III/GLASS-III/figures/analysis/diff_tumor_mac_heatmap_idhwt.pdf",width=7,height=7, useDingbats = FALSE)
grid.draw(g)
dev.off()
...
pdf("/projects/varnf/GLASS-III/GLASS-III/figures/analysis/diff_tumor_mac_heatmap_idhmut.pdf",width=7*sf,height=7, useDingbats = FALSE)
grid.draw(g)
dev.off()
...
pdf("/projects/varnf/GLASS-III/GLASS-III/figures/analysis/diff_tumor_heatmap_labelled.pdf",width=7,height=7, useDingbats = FALSE)
grid.draw(g)
dev.off()
...
fisher.test(ct)
# P = 0.03 for Mesenchymal
# P = 0.46 for Proneural
# P = 0.01 for Classical
...
c1 <- sum(idhmut_res[,"signature_name"] == "Mesenchymal" & idhmut_res[,"cluster"] == 1)
c2 <- sum(idhmut_res[,"signature_name"] == "Mesenchymal" & idhmut_res[,"cluster"] == 2)
c3 <- sum(idhmut_res[,"signature_name"] != "Mesenchymal" & idhmut_res[,"cluster"] == 1)
c4 <- sum(idhmut_res[,"signature_name"] != "Mesenchymal" & idhmut_res[,"cluster"] == 2)
ct <- matrix(c(c1,c2,c3,c4),nrow=2,ncol=2)
fisher.test(ct)
# P = 0.50 for Mesenchymal
# P = 1 for Proneural
# P = 1 for Classical