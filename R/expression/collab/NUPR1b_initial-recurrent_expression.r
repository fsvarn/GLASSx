...
g1 <- as.numeric(dat["ENST00000324873",info[which(info[,"idh_codel_subtype"]=="IDHwt"),"tumor_barcode_a"]])
g2 <- as.numeric(dat["ENST00000324873",info[which(info[,"idh_codel_subtype"]=="IDHwt"),"tumor_barcode_b"]])
idhwt_pval <- formatC(wilcox.test(g1,g2,paired=TRUE)$p.value, digits = 2)
idhwt_pval <- paste(idhwt_pval,"0",sep="")
mean(g1)
...
g1 <- as.numeric(dat["ENST00000324873",info[which(info[,"idh_codel_subtype"]!="IDHwt"),"tumor_barcode_a"]])
g2 <- as.numeric(dat["ENST00000324873",info[which(info[,"idh_codel_subtype"]!="IDHwt"),"tumor_barcode_b"]])
idhmut_pval <- formatC(wilcox.test(g1,g2,paired=TRUE)$p.value, digits = 2)
mean(g1)
sd(g1)/sqrt(length(g1))
mean(g2)
sd(g2)/sqrt(length(g2))
...
tpm <- t(dat["ENST00000324873",aliquot_barcode])
...
pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/ENST00000324873_longitudinal_boxplot.pdf",width=4,height=2)
#pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/ENST00000324873_longitudinal_boxplot_all_samps.pdf",width=4,height=2)
p1 <- ggplot(plot_dat, aes(x = timepoint, y = tpm)) +
geom_line(size=0.8,alpha=0.4,aes(group=case_barcode,colour= idh_codel_subtype)) +
geom_point(size=1) +
facet_grid(.~idhmut) +
labs(main = "ENST00000324873", y = "log10(TPM)") +
...
pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/ENST00000324873_longitudinal_mesenchymal_boxplot.pdf",width=1.5,height=2)
#pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/ENST00000324873_longitudinal_mesenchymal_boxplot_all.pdf",width=1.5,height=2)
p1 <- ggplot(plot_dat, aes(x = timepoint, y = tpm)) +
...
diff <- dat["ENST00000324873",info[,"tumor_barcode_b"]] - dat["ENST00000324873",info[,"tumor_barcode_a"]]
