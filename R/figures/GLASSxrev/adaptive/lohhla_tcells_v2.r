###################################################
# Examine association between HLA LOH and T cells
...
wt_p.val_a <- wt_p.val_b <- mut_p.val_a <- mut_p.val_b <- cod_p.val_a <- cod_p.val_b <- rep(0, length(cell_states))
for(i in 1:length(cell_states))
{
	g1 <- dat %>% filter(loss_a == "No HLA LOH", cell_state == cell_states[i], idh_codel_subtype == "IDHwt")  %>% .$fraction_a
	g2 <- dat %>% filter(loss_a == "HLA LOH", cell_state == cell_states[i], idh_codel_subtype == "IDHwt")  %>% .$fraction_a
	wt_p.val_a[i] <- t.test(g1,g2)$p.value

	g1 <- dat %>% filter(loss_b == "No HLA LOH", cell_state == cell_states[i], idh_codel_subtype == "IDHwt")  %>% .$fraction_b
	g2 <- dat %>% filter(loss_b == "HLA LOH", cell_state == cell_states[i], idh_codel_subtype == "IDHwt")  %>% .$fraction_b
	wt_p.val_b[i] <- t.test(g1,g2)$p.value

	g1 <- dat %>% filter(loss_a == "No HLA LOH", cell_state == cell_states[i], idh_codel_subtype == "IDHmut-noncodel")  %>% .$fraction_a
	g2 <- dat %>% filter(loss_a == "HLA LOH", cell_state == cell_states[i], idh_codel_subtype == "IDHmut-noncodel")  %>% .$fraction_a
	mut_p.val_a[i] <- t.test(g1,g2)$p.value

	g1 <- dat %>% filter(loss_b == "No HLA LOH", cell_state == cell_states[i], idh_codel_subtype == "IDHmut-noncodel")  %>% .$fraction_b
	g2 <- dat %>% filter(loss_b == "HLA LOH", cell_state == cell_states[i], idh_codel_subtype == "IDHmut-noncodel")  %>% .$fraction_b
	mut_p.val_b[i] <- t.test(g1,g2)$p.value

	g1 <- dat %>% filter(loss_a == "No HLA LOH", cell_state == cell_states[i], idh_codel_subtype == "IDHmut-codel")  %>% .$fraction_b
	g2 <- dat %>% filter(loss_a == "HLA LOH", cell_state == cell_states[i], idh_codel_subtype == "IDHmut-codel")  %>% .$fraction_b
	cod_p.val_a[i] <- t.test(g1,g2)$p.value
	
	# Not enough values
# 	g1 <- dat %>% filter(loss_b == "No HLA LOH", cell_state == cell_states[i], idh_codel_subtype == "IDHmut-codel")  %>% .$fraction_b
# 	g2 <- dat %>% filter(loss_b == "HLA LOH", cell_state == cell_states[i], idh_codel_subtype == "IDHmut-codel")  %>% .$fraction_b
# 	cod_p.val_b[i] <- t.test(g1,g2)$p.value
}

pvals <- data.frame(cell_states, wt_p.val_a, wt_p.val_b, mut_p.val_a, mut_p.val_b, cod_p.val_a)
...
test_dat2 <- plot_dat %>% filter(cell_state == "t_cell", idh_codel_subtype == "IDHmut-noncodel")
g1 <- test_dat2[which(test_dat2$loh_status == "HLA LOH"),"fraction"]
g2 <- test_dat2[which(test_dat2$loh_status == "No HLA LOH"),"fraction"]
t.test(g1,g2)

test_dat3 <- plot_dat %>% filter(cell_state == "t_cell", idh_codel_subtype == "IDHmut-codel")
g1 <- test_dat3[which(test_dat2$loh_status == "HLA LOH"),"fraction"]
g2 <- test_dat3[which(test_dat2$loh_status == "No HLA LOH"),"fraction"]
t.test(g1,g2)

pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/hla_loss_t_cells_v2.pdf",width=2,height=1.8)
...
facet_grid(.~idh_codel_subtype, scale = "free_y") +
theme_classic() +
theme(
