###################################################
# Create stacked barplots (transcriptional classifier/simplicity score/CIBERSORTx) of each GLASS sample using Neftel and eventually Venteicher fractions
# Updated: 2020.07.06
...
plot_res <- plot_res %>%
	mutate(cell_state = recode(cell_state, "T" = "T cell")) %>%
	mutate(cell_state = as_factor(cell_state)) %>%
	mutate(cell_state = fct_relevel(cell_state, "T cell","Macrophage", "Oligodendrocyte", "Mesenchymal","AClike","NPClike","OPClike"))
...
gg_simplicity_score <-
  ggplot(plot_res %>% filter(signature_name=="Mesenchymal", cell_state == "Mesenchymal"), aes(x=case_barcode)) +
  geom_tile(aes(y=status, alpha= -1 * ss)) +
  scale_fill_manual(values=c("black")) +
  theme(axis.title.y=element_blank())
  
gg_transcript_subtype_pro <-
  ggplot(plot_res %>% filter(signature_name=="Proneural", cell_state == "Mesenchymal"), aes(x=case_barcode)) +
  geom_tile(aes(y=status, fill=signature_name, alpha= pval)) +
  scale_fill_manual(values=c("#00458a")) +
  theme(axis.title.y=element_blank())
  
gg_transcript_subtype_cla <-
  ggplot(plot_res %>% filter(signature_name=="Classical", cell_state == "Mesenchymal"), aes(x=case_barcode)) +
  geom_tile(aes(y=status, fill=signature_name, alpha= pval)) +
...
gg_cell_state_init <- 
  ...
  scale_fill_manual(values=c("T cell" = "#bdd7e7", "Macrophage" = "#08519c",
						 "Oligodendrocyte" = "#2ca25f",
						 "AClike" = "#fcbba1",
						 "OPClike" = "#a50f15", "Mesenchymal" = "#fee391", "NPClike" = "#fb6a4a")) +
  theme(axis.title.y=element_blank())
...
gg_subtype_switch <- 
  ggplot(plot_res %>% filter(status=="Initial",signature_name=="Proneural", cell_state == "Mesenchymal"), aes(x=case_barcode)) +
  geom_tile(aes(y=signature_name, fill=switch)) +
  scale_fill_manual(values=c("white","black")) +
  theme(axis.title.y=element_blank())
  
gg_grade_change <- 
  ggplot(plot_res %>% filter(status=="Recurrent",signature_name=="Proneural", cell_state == "Mesenchymal"), aes(x=case_barcode)) +
...
#Plot
pdf("/projects/varnf/GLASS-III/GLASS-III/figures/analysis/transcriptional_subtype_change_neftel.pdf",width=7,height=5)
grid.draw(g)
dev.off()

#Legends
pdf("/projects/varnf/GLASS-III/GLASS-III/figures/analysis/transcriptional_subtype_change_neftel_legends_v3.pdf",width=7,height=7)
...
pdf("/projects/varnf/GLASS-III/GLASS-III/figures/analysis/cibersortx_stacked_barplot_timepoint_subtype_neftel.pdf",width=4.5,height=3.5)  
