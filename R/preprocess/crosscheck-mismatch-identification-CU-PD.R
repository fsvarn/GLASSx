...
workingdir = '/projects/varnf/GLASS-III/GLASS-III/results/fingerprinting/'
setwd(workingdir)
...
#CU-PD specific selection
cross_check_results = cross_check_results %>% 
	filter(grepl("GLSS-CU-P",LEFT_GROUP_VALUE) | grepl("GLSS-CU-P",RIGHT_GROUP_VALUE))
...
write.table(unmatched_aliquots, file = "GLASS-III/unmatched_aliquots.txt", sep="\t", row.names = F, col.names = T, quote = F)
write.table(mismatched_aliquots, file = "GLASS-III/mismatched_aliquots.txt", sep="\t", row.names = F, col.names = T, quote = F)
