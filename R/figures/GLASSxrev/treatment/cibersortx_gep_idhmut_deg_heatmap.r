###################################################
# Compare initial and recurrent genes in each cell type from initial and recurrent IDH-mutant tumors
...
#quantile(plot_res$expr,.99) #0.28, but using IDHwt cutoff so plots can be compared
...
# Plot the results (diff)
pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/idhmut_rec_sig_functional_enrichment_diff.pdf",width=3.75,height=2.2)
...
plotRes2 <- goRes %>%
		   filter(q.value < 0.1 & Significant > Expected) %>%
		   filter(cell_state == "stemcell_tumor")


plotRes2[,"logP"] <- -log10(as.numeric(plotRes2$q.value))

termord <- rev(as.character(plotRes2[order(plotRes2$logP, decreasing=TRUE),"Term"]))

plotRes2 <- plotRes2 %>% 
		   mutate(Term = fct_relevel(Term, termord))
	   
# Plot the results (stem)
pdf("/projects/verhaak-lab/GLASS-III/figures/analysis/idhmut_rec_sig_functional_enrichment_stem.pdf",width=2.75,height=1.8)
ggplot(data = plotRes2, aes(x = logP, y = Term, fill = cell_state)) +
geom_bar(stat="identity",position=position_dodge()) +
geom_vline(xintercept=-log10(0.10),linetype=2) +
scale_fill_manual(values = c("stemcell_tumor" = "#fb6a4a", "differentiated_tumor" = "#fcbba1", "prolif_stemcell_tumor" = "#a50f15")) + 
labs(x="-log10(adj. P-value)") +
#facet_wrap(grp~.,scales="free_y", ncol = 1) +
theme_classic() +
