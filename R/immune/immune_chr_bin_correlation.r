...
JOIN analysis.gatk_bin_diff_call an ON an.tumor_barcode_a = ps.dna_barcode_a AND an.tumor_barcode_b = ps.dna_barcode_b
...
mycor[which(cor_pval_adj[,1] < 0.1),]
inc_eff[which(inc_pval_adj[,1] < 0.1),]
del_eff[which(del_pval_adj[,1] < 0.1),]
...
pdf("/projects/varnf/GLASS-III/GLASS-III/figures/analysis/immune_cnv_bin_cor_bar.pdf",width=2,height=5.5)
